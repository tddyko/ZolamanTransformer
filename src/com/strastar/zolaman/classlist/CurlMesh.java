...
//			mBitmap = BitmapFactory.decodeResource(getResources(), mPhotos[position], mOptions);
//			mBitmap = BitmapFactory.
//	        Log.d(TAG, String.format("[after] w=%d h=%d option=%d", mBitmap.getWidth(),mBitmap.getHeight(),mOptions.inSampleSize));
//	        mBitmap = Bitmap.createScaledBitmap(mBitmap, mBitmap.getWidth(), mBitmap.getHeight(), true);
	        
			if(Common.DEBUG) Log.d("ST_CURL", 
					String.format("w(%d) h(%d) newW(%d) newH(%d)",w,h,newW,newH));
//			mBitmap = Bitmap.createBitmap(newW, newH, bitmap.getConfig());
			if(mBitmap!=null) mBitmap.recycle();
//			mBitmap = Bitmap.createScaledBitmap(bitmap, newW, newH, true);
			// 8888 은 알파값이 있고, 표현색상이 풍부합니다.
            // 565 는 알파값이 없고, 표현색상이 줄어듭니다.
            // 4444 는 알파값이 있고, 표현색상이 가장 적습니다.
			mBitmap = Bitmap.createBitmap(newW, newH, bitmap.getConfig().ARGB_4444);
...
//			Canvas c = new Canvas(bitmap);
	...
	public Bitmap getResizedBitmap(Bitmap bm, int newHeight, int newWidth) {
        ...
        // RESIZE THE BIT MAP
        matrix.postScale(newHeight, newHeight);
